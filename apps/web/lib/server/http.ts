...
/**
 * The second argument Next.js App Router passes to HTTP route handlers.
 *
 * In Next.js 15, `params` is a Promise that resolves to the dynamic route
 * segment values (route params became async in Next.js 15 and MUST be awaited).
 * Static routes receive the same context object, with `params` resolving to
 * `{}`; dynamic routes resolve to the segment map, e.g. `{ id: "…" }` for
 * `/api/assets/[id]`.
 *
 * This parameter MUST stay required (never `segment?: …`): Next.js 15
 * generates strict route-type validation (`.next/types/**\/route.ts`) that
 * compares the handler's second argument (`SecondArg`) against its
 * `RouteContext` (`{ params: Promise<…> }`). An optional parameter adds
 * `undefined` to the union and fails that `extends` check — the build error
 * seen at `app/api/assets/[id]/assign/route.ts`.
 */
