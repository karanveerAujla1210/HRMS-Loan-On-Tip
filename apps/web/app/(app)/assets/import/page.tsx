...
export default function AssetImportPage() {
  ...
  useEffect(() => {
    async function loadLookups() {
      ...
      const lookups = await fetchAssetSetupLookups(companyId);
      const toMap = (data: { id: string; name: string }[] | null) =>
        Object.fromEntries((data ?? []).map((r) => [r.name.toLowerCase().trim(), r.id]));
      setCategories(toMap(lookups.categories as { id: string; name: string }[]));
      setLocations(toMap(lookups.locations as { id: string; name: string }[]));
      setLookupReady(true);
    }
    void loadLookups();
  }, [companyId]);
