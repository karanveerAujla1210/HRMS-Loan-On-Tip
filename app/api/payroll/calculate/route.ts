...
export const POST = withApi<typeof CalculateSchema>({
  permission: "payroll.calculate",
  body: CalculateSchema,
  idempotencyEndpoint: "payroll/calculate",
  rateLimit: { limit: 10, windowMs: 60_000 },
  handler: async ({ ctx, body, audit, requestId }) => {
    const companyId = ctx.companyId!;
    const payroll_run_id = body.payroll_run_id;
    ...
    if ((run as { status: string }).status !== "DRAFT") {
      throw mapDatabaseError(
        Object.assign(
          new Error("HRMS:PAYROLL_INVALID_STATE:Only DRAFT runs can be calculated."),
          { code: "P0001" }
        )
      );
    }
    ...
    for (const emp of (employees ?? []) as EmpRow[]) {
      ...
      if (structureId) {
        ...
        if (compErr) throw mapDatabaseError(compErr);
      ...
      const { data: item, error: itemErr } = await db
        ...
        .select("id")
        .single();
      if (itemErr) throw mapDatabaseError(itemErr);
      ...
      if (lineItems.length) {
        ...
        if (compUpsertErr) throw mapDatabaseError(compUpsertErr);
      }
      ...
      if (slipErr) throw mapDatabaseError(slipErr);
    ...
    const { error: updErr } = await db
      ...
      .eq("id", payroll_run_id)
      .eq("status", "DRAFT") // conditional update: fails if state changed concurrently
    ...
    await audit({
      ...
      newValues: { employee_count: count, net_pay: netPay, engine_version: PAYROLL_DEFAULTS.engineVersion },
    });
